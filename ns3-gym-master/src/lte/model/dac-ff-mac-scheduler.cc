...
#include <ns3/dac-ff-mac-scheduler.h>
...
namespace ns3 {
  ...
  vector<string> 
  split1(const string& str, const string& delim) 
  {  
    vector<string> res;  
    if("" == str) return res;  
...
NS_LOG_COMPONENT_DEFINE ("DacFfMacScheduler");
...
NS_OBJECT_ENSURE_REGISTERED (DacFfMacScheduler);



DacFfMacScheduler::DacFfMacScheduler ()
  : m_cschedSapUser (0),
    m_schedSapUser (0),
    m_timeWindow (99.0),
    m_nextRntiUl (0)
{
  ...
  m_cschedSapProvider = new MemberCschedSapProvider<DacFfMacScheduler> (this);
  m_schedSapProvider = new MemberSchedSapProvider<DacFfMacScheduler> (this);
  m_ffrSapProvider = 0;
  m_ffrSapUser = new MemberLteFfrSapUser<DacFfMacScheduler> (this);
}
...
TypeId
DacFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DacFfMacScheduler")
    .SetParent<FfMacScheduler> ()
    .SetGroupName("Lte")
    .AddConstructor<DacFfMacScheduler> ()
    .AddAttribute ("CqiTimerThreshold",
                   "The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
                   UintegerValue (1000),
                   MakeUintegerAccessor (&DacFfMacScheduler::m_cqiTimersThreshold),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("HarqEnabled",
                   "Activate/Deactivate the HARQ [by default is active].",
                   BooleanValue (false),
                   MakeBooleanAccessor (&DacFfMacScheduler::m_harqOn),
                   MakeBooleanChecker ())
    .AddAttribute ("UlGrantMcs",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DacFfMacScheduler::m_ulGrantMcs),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("test",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   StringValue (),
                   MakeStringAccessor (&DacFfMacScheduler::m_test),
                   MakeStringChecker ()) 
    .AddAttribute ("cellid",
                   "cellid",
                   StringValue ("0"),
                   MakeStringAccessor (&DacFfMacScheduler::m_cellid),
                   MakeStringChecker ()) 
    // .AddAttribute ("m_vec",
    //                "The MCS of the UL grant, must be [0..15] (default 0)",
    //                ObjectVectorValue (),
    //                MakeObjectVectorAccessor (&DacFfMacScheduler::m_vec),
    //                MakeObjectVectorChecker<uint32_t> ()) 
    // .AddAttribute ("test",
    //                "The MCS of the UL grant, must be [0..15] (default 0)",
    //                ObjectMapValue (),
    //                MakeObjectMapAccessor (&DacFfMacScheduler::m_flowStatsDl),
    //                MakeObjectMapChecker<dacsFlowPerf_t> ())                                                                  
  ;
  return tid;
}
...
void
DacFfMacScheduler::SetFfMacCschedSapUser (FfMacCschedSapUser* s)
{
  m_cschedSapUser = s;
}

void
DacFfMacScheduler::SetFfMacSchedSapUser (FfMacSchedSapUser* s)
{
  m_schedSapUser = s;
}

FfMacCschedSapProvider*
DacFfMacScheduler::GetFfMacCschedSapProvider ()
{
  return m_cschedSapProvider;
}

FfMacSchedSapProvider*
DacFfMacScheduler::GetFfMacSchedSapProvider ()
{
  return m_schedSapProvider;
}

void
DacFfMacScheduler::SetLteFfrSapProvider (LteFfrSapProvider* s)
{
  m_ffrSapProvider = s;
}

LteFfrSapUser*
DacFfMacScheduler::GetLteFfrSapUser ()
{
  return m_ffrSapUser;
}

void
DacFfMacScheduler::DoCschedCellConfigReq (const struct FfMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoCschedUeConfigReq (const struct FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
...
void
DacFfMacScheduler::DoCschedLcConfigReq (const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this << " New LC, rnti: "  << params.m_rnti);

  std::map <uint16_t, dacsFlowPerf_t>::iterator it;
      ...
      if (it == m_flowStatsDl.end ())
        {
          ...
          m_flowStatsDl.insert (std::pair<uint16_t, dacsFlowPerf_t> (params.m_rnti, flowStatsDl));
          dacsFlowPerf_t flowStatsUl;
          ...
          m_flowStatsUl.insert (std::pair<uint16_t, dacsFlowPerf_t> (params.m_rnti, flowStatsUl));
        }
    }
...
void
DacFfMacScheduler::DoCschedLcReleaseReq (const struct FfMacCschedSapProvider::CschedLcReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoCschedUeReleaseReq (const struct FfMacCschedSapProvider::CschedUeReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoSchedDlRlcBufferReq (const struct FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
...
void
DacFfMacScheduler::DoSchedDlPagingBufferReq (const struct FfMacSchedSapProvider::SchedDlPagingBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoSchedDlMacBufferReq (const struct FfMacSchedSapProvider::SchedDlMacBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  NS_FATAL_ERROR ("method not implemented");
  return;
...
int
DacFfMacScheduler::GetRbgSize (int dlbandwidth)
{
  for (int i = 0; i < 4; i++)
    {
...
unsigned int
DacFfMacScheduler::LcActivePerFlow (uint16_t rnti)
{
  std::map <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
uint8_t
DacFfMacScheduler::HarqProcessAvailability (uint16_t rnti)
{
...
uint8_t
DacFfMacScheduler::UpdateHarqProcessId (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
  if (m_harqOn == false)
    {
...
void
DacFfMacScheduler::RefreshHarqProcesses ()
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  if (rbgAllocatedNum == rbgNum)
    ...
    vector<string> vs = split1(m_test," ");//分割字符串
  ...
  std::map <uint16_t, dacsFlowPerf_t>::iterator itStats;
...
void
DacFfMacScheduler::DoSchedDlRachInfoReq (const struct FfMacSchedSapProvider::SchedDlRachInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoSchedDlCqiInfoReq (const struct FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
double
DacFfMacScheduler::EstimateUlSinr (uint16_t rnti, uint16_t rb)
{
...
void
DacFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
...
void
DacFfMacScheduler::DoSchedUlNoiseInterferenceReq (const struct FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
DacFfMacScheduler::DoSchedUlSrInfoReq (const struct FfMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
DacFfMacScheduler::DoSchedUlMacCtrlInfoReq (const struct FfMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
DacFfMacScheduler::DoSchedUlCqiInfoReq (const struct FfMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
// retrieve the allocation for this subframe
  ...
  switch (params.m_ulCqi.m_type)
    ...
    case UlCqi_s::PRACH:
      {
        NS_FATAL_ERROR ("DacFfMacScheduler supports only PUSCH and SRS UL-CQIs");
      }
      break;
    default:
...
void
DacFfMacScheduler::RefreshDlCqiMaps (void)
{
...
void
DacFfMacScheduler::RefreshUlCqiMaps (void)
{
  // refresh UL CQI  Map
...
void
DacFfMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid, uint16_t size)
{
  std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
void
DacFfMacScheduler::UpdateUlRlcBufferInfo (uint16_t rnti, uint16_t size)
{
...
void
DacFfMacScheduler::TransmissionModeConfigurationUpdate (uint16_t rnti, uint8_t txMode)
{
