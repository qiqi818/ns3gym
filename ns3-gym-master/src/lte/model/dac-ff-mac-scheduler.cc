...
static const int DacType0AllocationRbg[4] = {
  10,       // RGB size 1
  26,       // RGB size 2
  63,       // RGB size 3
  110       // RGB size 4
...
DacFfMacScheduler::GetRbgSize (int dlbandwidth)
{
  for (int i = 0; i < 4; i++)
    {
      if (dlbandwidth < DacType0AllocationRbg[i])
        {
          return (i + 1);
        }
    }
...
DacFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  if (rbgAllocatedNum == rbgNum)
    ...
    //改写的下行调度过程，将传递进来的保存有动作（资源分配信息的）参数m_test转化为调度信息执行下去
            
    vector<string> vs = split(m_test," ");//分割字符串
      ...
      if (LcActivePerFlow (stoi(vs[i])) > 0)
      {
        ...
        if (itMap == allocationMap.end ())
        ...
        else
          {
            ...
            //     if(count < vs.size())
            //     {
            //       if (LcActivePerFlow (ii) > 0)
            //       {
                    
            //         rbgMap.at (stoi(vs[count])) = true;
                  
            //         std::map <uint16_t, std::vector <uint16_t> >::iterator itMap;
            //         itMap = allocationMap.find (ii);
            //         if (itMap == allocationMap.end ())
            //           {
            //             // insert new element
            //             std::vector <uint16_t> tempMap;
            //             tempMap.push_back (stoi(vs[count]));
                        
            //             allocationMap.insert (std::pair <uint16_t, std::vector <uint16_t> > (ii, tempMap));
            //           }
            //         else
            //           {
            //             (*itMap).second.push_back (stoi(vs[count]));
