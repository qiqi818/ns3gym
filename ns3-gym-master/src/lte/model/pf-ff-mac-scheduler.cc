...
PfFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PfFfMacScheduler")
    ...
    .AddAttribute ("test",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   StringValue (),
                   MakeStringAccessor (&PfFfMacScheduler::m_test),
                   MakeStringChecker ()) 
    .AddAttribute ("cellid",
                   "cellid",
                   StringValue ("0"),
                   MakeStringAccessor (&PfFfMacScheduler::m_cellid),
                   MakeStringChecker ()) 
    // .AddAttribute ("m_vec",
    //                "The MCS of the UL grant, must be [0..15] (default 0)",
    //                ObjectVectorValue (),
    //                MakeObjectVectorAccessor (&PfFfMacScheduler::m_vec),
    //                MakeObjectVectorChecker<uint32_t> ()) 
    // .AddAttribute ("test",
    //                "The MCS of the UL grant, must be [0..15] (default 0)",
    //                ObjectMapValue (),
    //                MakeObjectMapAccessor (&PfFfMacScheduler::m_flowStatsDl),
    //                MakeObjectMapChecker<pfsFlowPerf_t> ())                                                                  
  ;
  return tid;
}
