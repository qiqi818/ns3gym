...
#include <cfloat>
#include <set>
#include "async-ff-mac-scheduler.h"
#include "string.h"
using namespace std;
namespace ns3 {
...
AsyncFfMacScheduler::RefreshHarqProcesses ()
{
  ...
  {
    uint16_t rnti = (*itTimers).first;
    DlHarqProcessesTimer_t &timers = (*itTimers).second;
    for (uint16_t i = 0; i < HARQ_PROC_NUM; i++)
    {
      if (timers.at (i) == HARQ_DL_TIMEOUT) {
        // reset HARQ process
...
AsyncFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  for (uint16_t i = 0; i < m_dlInfoListBuffered.size (); i++) 
  {
    ...
    if (retx.at (0) || retx.at (1)) // need retx
    {
