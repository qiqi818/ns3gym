...
Ptr<OpenGymDataContainer> MyGetObservation(void)
{
  ...
  // PointerValue pptr;
  // ueDevs.Get(0) -> GetObject<LteEnbNetDevice> ()->;
  // cout << ppt
  // if(fff->m_epsBearer.qci == EpsBearer::GBR_GAMING)
  //   cout << "1++-+-+-+-+" << endl;
  // else
  //   cout << "++-+-+--+----/*--*"<<endl;

  //cout << ueDevs.Get(0)->GetObject<LteDataRadioBearerInfo> ()->m_epsBearer.qci << endl;
  cout << Simulator::Now() << endl;
  uint32_t nodeNum = 7;
  std::vector<uint32_t> shape = {nodeNum,};
  Ptr<OpenGymBoxContainer<uint32_t> > box = CreateObject<OpenGymBoxContainer<uint32_t> >(shape);
  // PointerValue pptr;
  // enbDevs.Get(0)->GetAttribute("EpsBearer::Qci",pptr);
  // enbDevs.Get(0)->GetAttribute("EpsBearer::Qci")
  list<list<int>> ls;
  ...
  {    
    ...
    for(std::map <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it = pff->m_rlcBufferReq.begin(); it != pff->m_rlcBufferReq.end(); it++)
    {
    ...
    // auto f = [](list<int> a, list<int>b)
    // {
    //   return a.front()>b.front();
    // };
    // sort(ls.begin(), ls.end(), f);
    ls.sort();
    ls.reverse();
    //  for(int m = 0;  m < 50; m++)
    //  {
...
float MyGetReward(void)
{
  ...
  //reward = 1;
  ifstream file;
  file.open("/home/liqi/ns3-gym/ns3-gym-master/DlRxPhyStats.txt");
  char szbuff[1024] = {0};
  string za;
  float txx = 0;
  Time tim = Simulator::Now();
  
  while(!file.eof()){
    file.getline(szbuff,1024);
    za = szbuff;
    vector<string> sp = split(za.c_str()," ");
    if(sp.size() == 12 && atoi(sp[0].c_str()) == (tim.GetMilliSeconds()-1) && atoi(sp[10].c_str()) == 1){
      txx += atoi(sp[7].c_str());
    }
  }
  reward = txx/0.001;
