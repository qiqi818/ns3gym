...
cmd.Parse (argc, argv);
  ...
  lteHelper->SetEnbDeviceAttribute ("DlBandwidth", UintegerValue (100));//6 15 25  50 75 100
  ...
  z->SetAttribute ("Min", DoubleValue (-500));
  z->SetAttribute ("Max", DoubleValue (500));
  ...
  enbNodes.Get (1)->GetObject<MobilityModel> ()->SetPosition (Vector(500,0,100));
    enbNodes.Get (2)->GetObject<MobilityModel> ()->SetPosition (Vector(250,430,100));
  enbNodes.Get (3)->GetObject<MobilityModel> ()->SetPosition (Vector(-250,430,100));
    enbNodes.Get (4)->GetObject<MobilityModel> ()->SetPosition (Vector(-500,0,100));
  enbNodes.Get (5)->GetObject<MobilityModel> ()->SetPosition (Vector(-250,-430,100));
    enbNodes.Get (6)->GetObject<MobilityModel> ()->SetPosition (Vector(250,-430,100));
...
//     ueNodes.Get (13)->GetObject<MobilityModel> ()->SetPosition (Vector(135,-215,0));
  ...
  // UDP connection from remote to UEnodes
    ...
    Ptr<UniformRandomVariable> start = CreateObject<UniformRandomVariable> ();
    start->SetAttribute ("Min", DoubleValue (1));
    start->SetAttribute ("Max", DoubleValue (20));

    Ptr<UniformRandomVariable> ktime = CreateObject<UniformRandomVariable> ();
    ktime->SetAttribute ("Min", DoubleValue (1));
    ktime->SetAttribute ("Max", DoubleValue (5));
    ...
    for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
      {
        ...
        clientApps.Get(u)->SetStopTime(Seconds (st+ktime->GetValue()));
      }
