...
class PolicyGradient:
    ...
    def learn(self):
        ...
        self.sess.run(self.train_op, feed_dict={
             self.X: np.vstack(self.episode_observations[0:len(self.episode_observations)-2]).T,
             self.Y: np.vstack(np.array(self.episode_actions[0:len(self.episode_actions)-2])).T,
             self.discounted_episode_rewards_norm: discounted_episode_rewards_norm[2:],
        })

        # Reset the episode data
        self.episode_observations = self.episode_observations[len(self.episode_observations)-2:]
        self.episode_actions = self.episode_actions[len(self.episode_actions)-2:]
        self.episode_rewards = self.episode_rewards[0:2]
